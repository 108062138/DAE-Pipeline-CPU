...
MY_MERGESORT_RTOS_BUILD="${MY_MERGESORT_RTOS_BUILD:-$ROOT/build/my_mergesort_rtos}"
MY_MERGESORT_RTOS_ELF="${MY_MERGESORT_RTOS_ELF:-$MY_MERGESORT_RTOS_BUILD/my_mergesort_rtos}"
MAX_CYCLES="${MAX_CYCLES:-400000}"
SKIP_BUILD="${SKIP_BUILD:-0}"
SKIP_MY_MERGESORT_RTOS_BUILD="${SKIP_MY_MERGESORT_RTOS_BUILD:-0}"
...
if [[ "$SKIP_MY_MERGESORT_RTOS_BUILD" != "1" ]]; then
  cmake -S "$ROOT/my_program/my_mergesort_rtos" -B "$MY_MERGESORT_RTOS_BUILD" \
    -DCMAKE_TOOLCHAIN_FILE="$ROOT/runtime/toolchain.cmake"
  cmake --build "$MY_MERGESORT_RTOS_BUILD" -j
fi

if [[ ! -f "$MY_MERGESORT_RTOS_ELF" ]]; then
  echo "Missing my_mergesort_rtos ELF: $MY_MERGESORT_RTOS_ELF" >&2
  exit 1
fi
...
"$SIM" +ELF="$MY_MERGESORT_RTOS_ELF" +MAX_CYCLES="$MAX_CYCLES" +QUIET
