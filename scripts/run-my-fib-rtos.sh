...
MY_FIB_RTOS_BUILD="${MY_FIB_RTOS_BUILD:-$ROOT/build/my_fib_rtos}"
MY_FIB_RTOS_ELF="${MY_FIB_RTOS_ELF:-$MY_FIB_RTOS_BUILD/my_fib_rtos}"
MAX_CYCLES="${MAX_CYCLES:-200000}"
SKIP_BUILD="${SKIP_BUILD:-0}"
SKIP_MY_FIB_RTOS_BUILD="${SKIP_MY_FIB_RTOS_BUILD:-0}"
...
if [[ "$SKIP_MY_FIB_RTOS_BUILD" != "1" ]]; then
  cmake -S "$ROOT/my_program/my_fib_rtos" -B "$MY_FIB_RTOS_BUILD" \
    -DCMAKE_TOOLCHAIN_FILE="$ROOT/runtime/toolchain.cmake"
  cmake --build "$MY_FIB_RTOS_BUILD" -j
fi

if [[ ! -f "$MY_FIB_RTOS_ELF" ]]; then
  echo "Missing my_fib_rtos ELF: $MY_FIB_RTOS_ELF" >&2
  exit 1
fi
...
"$SIM" +ELF="$MY_FIB_RTOS_ELF" +MAX_CYCLES="$MAX_CYCLES" +QUIET
